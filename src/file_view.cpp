...
// void FileView::FilterContext::update() {
// 	uint64_t active_ctxs_mask = (1ULL << find_ctxs.size()) - 1;
// 	const bool and_ = true;
//
// 	while (1) {
// 		auto min_line_idx = std::numeric_limits<size_t>::max();
// 		auto mask = active_ctxs_mask;
// 		while (mask) {
// 			size_t idx = std::countr_zero(mask);
// 			mask &= ~(1ULL << idx);
// 			auto &ctx = find_ctxs[idx];
//
// 			if (ctx.next_array_idx >= ctx.find->line_indices.size()) {
// 				active_ctxs_mask &= ~(1ULL << idx);
// 				continue;
// 			}
// 			min_line_idx = std::min(min_line_idx, ctx.find->line_indices[ctx.next_array_idx]);
// 		}
//
// 		if (!active_ctxs_mask) {
// 			break;
// 		}
//
// 		size_t num_active_ctxs = std::popcount(active_ctxs_mask);
// 		mask = active_ctxs_mask;
...
// 			auto &ctx = find_ctxs[idx];
//
// 			if (ctx.find->line_indices[ctx.next_array_idx] == min_line_idx) {
// 				num_matches++;
// 				ctx.next_array_idx++;
// 			}
// 		}
//
// 		if ((!and_ && num_matches > 0) || (and_ && num_matches == num_active_ctxs)) {
// 			// add the line to the filtered indices
// 			filtered_line_indices_.push_back(min_line_idx);
// 		}
// 	}
// }
//
// void FileView::FilterContext::remove(const FindContext *ctx) {
// 	std::erase_if(find_ctxs, [&](const Ctx &ele) {
// 		return ele.find == ctx;
// 	});
// 	filtered_line_indices_.resize_uninitialized(0);
// }
//
// void FileView::FilterContext::add(const FindContext *ctx) {
// 	find_ctxs.push_back({ctx, 0});
// 	filtered_line_indices_.resize_uninitialized(0);
// }
...
void FileView::on_findview_event(FindView &view, FindView::Event event) {
	switch (event) {
		case FindView::Event::kCriteria: {
			...
			auto &find_ctx = find_ctxs_.at(&view);

			finder_.remove(&view);

			find_ctx->reset();
			// line_filter_.remove(find_ctx.get());
			// line_filter_.add(find_ctx.get());
			content_view_.stripe_view_.remove_dataset(&view);
			content_view_.stripe_view_.add_dataset(&view, view.color());

			// active_filter_ = &line_filter_;

			HS_FLAG_CASELESS;     // /i  - Matching will be performed case-insensitively.
			HS_FLAG_DOTALL;       // /s  - Matching a `.` will not exclude newlines.
			HS_FLAG_MULTILINE;    // /m  - `^` and `$` anchors match any newlines in data.
			HS_FLAG_SINGLEMATCH;  // !/g - Only one match will be generated by patterns with this match id per stream.
			HS_FLAG_ALLOWEMPTY;   //     - Allow expressions which can match against an empty string, such as `.*`.
			HS_FLAG_UTF8;         // /u  - Treat this pattern as a sequence of UTF-8 characters.
			...
			int flags = 0;
			auto view_flags = view.flags();
			auto pattern = std::string{view.text()};

			if (view_flags.regex) {
				// Check if the pattern contains flags, i.e. '/' at start, and '/' before end
				auto first_slash = pattern.find_first_of('/');
				auto last_slash = pattern.find_last_of('/');
				if (pattern.size() > 2 && first_slash == 0 && last_slash != first_slash && last_slash < pattern.size() - 1) {
					// Extract flags from the pattern
					auto flags_str = pattern.substr(last_slash + 1);
					pattern = pattern.substr(1, last_slash - 1); // Remove the slashes
					...
					for (const char c : flags_str) {
						switch (c) {
							...
							default:
								std::cerr << "Unsupported regex flag: '" << c << "'" << std::endl;
								state.bad_pattern = true;
						}
					}
				} else {
					// No flags found, use default regex flags
					flags = HS_FLAG_MULTILINE;  // | HS_FLAG_DOTALL;
				}
			} else {
				// use default literal flags
			}

			if (view_flags.whole_word) {
				// Convert the pattern to a whole word search
				// This requires enabling regex mode
				view_flags.regex = true;
				pattern = "\\b" + pattern + "\\b";
			}

			flags |= view_flags.case_sensitive ? 0 : HS_FLAG_CASELESS;
			flags |= view_flags.regex ? Finder::FLAG_REGEX : 0;

			int ret = finder_.submit(&view, [this](auto ctx, auto idx){on_finder_results(ctx, idx);}, pattern, flags);
			if (ret != 0) {
				state.bad_pattern = true;
		...
		case FindView::Event::kFilter: {
			if (active_filter_ == find_ctxs_.at(&view).get()) {
				active_filter_ = nullptr;
			} else {
				active_filter_ = find_ctxs_.at(&view).get();
			}

			for (auto &[v, ctx] : find_ctxs_) {
				v->set_filtered(active_filter_ == ctx.get());
			}
...
size_t FileView::num_filtered_lines() const {
	if (!active_filter_) {
		return num_lines();
	}
	return active_filter_->line_indices.size();
}


ivec2 FileView::max_scroll() const {
	return TextShader::font().size * ivec2{longest_line_, num_filtered_lines()};
}
...
size_t FileView::abs_char_idx_to_buf_char_idx(size_t abs_idx) const {
	...
	auto it = std::lower_bound(content_view_.base_styles_.begin(), content_view_.base_styles_.end(), abs_idx,
		[this](const TextShader::CharStyle &style, size_t idx) {
			return abs_char_loc_to_abs_char_idx({style.char_pos.x, style.real_line_idx}) < idx;
		});

	if (it == content_view_.base_styles_.begin() && abs_idx < abs_char_loc_to_abs_char_idx({content_view_.base_styles_[0].char_pos.x, content_view_.base_styles_[0].real_line_idx})) {
...
// void FileView::update_filtered_lines() {
// 	ZoneScopedN("Update filtered lines");
// 	line_filter_.update();
// }
...
void FileView::really_update_buffers(const uint8_t *data) {
	...
	for (int buf_line_idx = std::max(0, content_view_.buf_char_window_.tl.y); buf_line_idx < std::clamp(content_view_.buf_char_window_.br.y, 0, (int)num_lines()); ++buf_line_idx) {
		size_t line_idx;
		if (active_filter_) {
			if (buf_line_idx >= active_filter_->line_indices.size()) {
				break; // No more lines to process
			}
			line_idx = active_filter_->line_indices[buf_line_idx];
		} else {
			line_idx = buf_line_idx;
		}
...
bool FileView::update_buffers(const Dataset::User &user) {
	...
	need_buffer_update_ |= true;
	need_buffer_update_ |= !content_view_.buf_char_window_.contains(visible_char_window);
...
void FileView::update() {
	...
	const auto prev_linenum_chars = linenum_view_.linenum_chars_;
